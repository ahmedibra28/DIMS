...
import type { Resource as IResource } from '@/prisma/generated/client'
...
const Page = () => {
  ...
  const uId =
    userInfo?.role === 'INSTRUCTOR' ? userInfo?.instructorId : userInfo?.id
  ...
  const getApi = useApi({
    key: ['resources', uId!],
    method: 'GET',
    url: `resources?page=${page}&q=${q}&limit=${limit}`,
  })?.get
  ...
  const editHandler = (
    item: IResource & { subject: { course: { id: string } } }
  ) => {
    setId(item.id!)
    setEdit(true)
